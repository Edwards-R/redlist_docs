---
parent: Wasp
---

# Aphelopus atratus: iso. Olmi: 1994

## Provisional Red List status: DD
- A2bc
- A3b
- D2

## Red List Justification
*N/A*

### Narrative
Also known as A. holommelas Richards, 1939 in Perkins (1976). Records of this dryinid wasp are from Devon to Kent and north to Cheshire and Co. Durham, Wales (Glamorgan, Pembrokeshire) and Scotland (Dumfries and Galloway, Lanarkshire, Edinburgh, Lothian, Angus, Highlands) which is almost certainly a true reflection of it's distribution. It acts as a parasitoid/predator on Typhloybinae,Cicadellidae (Leaf Hoppers) and has been reared from Lebra albastriella (Fallén), A. wahlbergi (Boheman), Eupteryx aurata Linnaeus), E. cyclops Matsumura, E. melissae (Curtis), E. urticae (Fabricius), E.hippocast (Edwards), Fagocyha cruenta (Herrich-Schäffer), Ribautiana tenerrima (Herrich-Schäffer), R. ulmi )Linnaeus) and also recorded on Typhlocyba opaca on apple.

Insufficient data were available to reach a supportable conclusion on the threat status for this taxon.

### Quantified Attributes
|Attribute|Result|
|---|---|
|Synanthropy|No|
|Vagrancy|No|
|Colonisation|No|
|Nomenclature|No|


## National Rarity
Insufficient Data (*ID*)

## National Presence
|Country|Presence
|---|:-:|
|England|Y|
|Scotland|N|
|Wales|N|


## Distribution map
![](../map/6.svg)

## Red List QA Metrics
### Decade
| Slice | # Records | AoO (sq km) | dEoO (sq km) |BU%A |
|---|---|---|---|---|
|1992 - 2001|3|8|9968|27%|
|2002 - 2011|16|28|26964|74%|
|2012 - 2021|3|12|14288|39%|

### 5-year
| Slice | # Records | AoO (sq km) | dEoO (sq km) |BU%A |
|---|---|---|---|---|
|2002 - 2006|13|20|14710|40%|
|2007 - 2011|3|8|13388|37%|
|2012 - 2016|1|4|6694|18%|
|2017 - 2021|2|8|13388|37%|

### Criterion A2 (Statistical)
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|Raw record count|LC|100%|No|Insufficient data|
|AoO|LC|100%|No|Insufficient data|
|dEoO|LC|100%|No|Insufficient data|
|Bayesian|DD|*NaN*%|Yes||
|Bayesian (Expert interpretation)|DD|*N/A*|Yes||

### Criterion A2 (Expert Inference)
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|Internal review|DD||Yes||

### Criterion A3 (Expert Inference)
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|Internal review|DD||Yes||

### Criterion B
|Criterion| Value|
|---|---|
|Locations|>10|
|Subcriteria||
|Support||

#### B1
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|MCP|LC|94650|No|Insufficient data|

#### B2
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|Tetrad|LC|48|No|Insufficient data|

### Criterion B (Natural England)
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|B1 (AoO)|LC|50|||

|B1 (dEoO)|LC|43.3386837881219904|||

|B2 (AoO)|LC|50|||

|B2 (dEoO)|LC|43.3386837881219904|||

### Criterion D2
|Attribute|Assessment|Value|Accepted|Justification
|---|---|---|---|---|
|D2|DD|*N/A*|Yes||

### Wider Review
|  |  |
|---|---|
|**Action**|Maintained|
|**Reviewed Status**|DD|
|**Justification**|Records of this genus are sporadic and tend to follow the activity of individual recorders / identifiers rather than any true distribution. Details of biology and ecology are imperfectly known.|

## National Rarity QA Metrics
|Attribute|Value|
|---|---|
|Hectads|10|
|Calculated|NR|
|Final|ID|
|Moderation support||
